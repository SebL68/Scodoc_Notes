    ...
    public static function updateLists(){
		...
		if ($id_LDAP = self::openLDAP()) {
            if ($Config->LDAP_filtre_ufr != '') {
                self::updateList($id_LDAP, $STUDENTS_PATH, "(&(".$Config->LDAP_filtre_statut_etudiant.")(".$Config->LDAP_filtre_ufr."))", [$Config->LDAP_uid]);
                ...
                self::updateList($id_LDAP, $STUDENTS_PATH, "(".$Config->LDAP_filtre_statut_etudiant.")", [$Config->LDAP_uid]);
