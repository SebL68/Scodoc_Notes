...
if (isset($_GET['q'])) {
	...
	switch ($_GET['q']) {
		...
		case 'semestresEtudiant':
			...
			if ($user->getStatut() < PERSONNEL && isset($_GET['etudiant'])) {
				returnError();
			}
		...
		case 'setAbsence':
			...
			if ($user->getStatut() < ADMINISTRATEUR && $_GET['statut'] == 'justifie') {
				returnError();
			}
		...
		case 'setJustifie':
			if ($user->getStatut() < ADMINISTRATEUR) {
				returnError();
			}
			sanitize($_GET['semestre']);
			sanitize($_GET['etudiant']);
		...
		/*************************/
		case 'updateLists':
			if ($user->getStatut() < SUPERADMINISTRATEUR) {
				returnError();
			}
		...
		case 'getStudentPic':
			...
			if (!file_exists($url)) { // Image par défaut si elle n'existe pas
				if (method_exists('Config', 'customPic') == true) {
					...
					echo '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24" fill="none" stroke="var(--fond-inverse)" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>';
					return;
				}
