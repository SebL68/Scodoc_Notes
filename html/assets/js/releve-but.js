...
<div>	
  ...
  showSemestre(data) {
    ...
    this.shadow.querySelector("h2").innerHTML +=
      data.semestre.numero + " - " + data.semestre.groupes[0]?.group_name || "";
    this.shadow.querySelector(".dateInscription").innerHTML += this.ISOToDate(
      data.semestre.inscription
    );
    let output = `
			<div>
				<div class=enteteSemestre>Moyenne</div><div class=enteteSemestre>${
          data.semestre.notes.value
        }</div>
				<div class=rang>Rang :</div><div class=rang>${data.semestre.rang.value} / ${
      data.semestre.rang.total
    }</div>
        ...
        output += `<div>
					<div class=enteteSemestre>${
            data.semestre.groupes[0]?.group_name
          }</div><div></div>
					<div class=rang>Rang :</div><div class=rang>${dataGroupe.value} / ${
          dataGroupe.total
        }</div>
    ...
    if (data.semestre.decision_annee?.code) {
      this.shadow.querySelector(".decision_annee").innerHTML =
        "Décision année : " +
        data.semestre.decision_annee.code +
        " - " +
        correspondanceCodes[data.semestre.decision_annee.code];
    }
  ...
  isNewEvaluation(evaluation) {
    const seenEvaluations = this.getSeenEvaluations();
    const index = seenEvaluations.findIndex(
      (e) => parseInt(e.id) === evaluation.id
    );
    if (index === -1) {
      return true;
    }
  ...
  evaluation(evaluations) {
    ...
    evaluations.forEach((evaluation) => {
      ...
      output += `
				<div class="eval ${isNewEvaluation ? "new-eval" : ""}" data-id="${
        evaluation.id
      }" data-note="${evaluation.note.value}">
