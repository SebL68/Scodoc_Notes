...
<body>
	...
	<main>
		<h2>24/02/2023 - 5.2.0</h2>
		<ul>
			<li>Ajout d'un système de thème clair/sombre</li>
		</ul>
		...
		<ul>
			<li>Configuration dans les requêtes : permet de modifier des options comme le téléchargement des relevés PDF
				sans avoir à changer la version du site (problématique de cache agressif).</li>
			<li>Diverses améliorations dans le code.</li>
		</ul>

		<h2>16/02/2023 - 5.0.9</h2>
		<ul>
			<li>Option : public static $CAS_nip_key = 'umCodeEtudiant'; /* Ceci est un exemple */ - Defaut : false -
				Permet de sélectionner le nip depuis un paramètre complémentaire du CAS, voir /code_test/testCAS.php
			</li>
		</ul>
		...
		<ul>
			<li>Ajout d'un bouton pour télécharger le relevé PDF à partir du relevé BUT - option modifiable dans
				config.php</li>
			<li>Prise en compte d'une nouvelle route Scodoc permettant d'éviter un bug lorsqu'un étudiant change de
				département dans la même année.</li>
		</ul>
		...
		<ul>
			...
			<li>Absences : utilisation du format heure à la place du format flottant pour l'affichage (8.5 -> 8h30).
			</li>
		...
		<ul>
			<b>Attention</b>
			<li>L'onglet "comptes" permet désormais de gérer tous les idCAS et pas que les mails, ⚠️⚠️⚠️ il est
				nécessaire d'exécuter le script /includes/CmdUpgradeUserListToV5.0.0.php ⚠️⚠️⚠️ - Merci Denis Graef.
			</li>
			...
			<br>
			<b>Ajouts</b>
			<li>Ajout d'un système d'analyse du trafic interne à la passerelle : à activer dans config.php - les
				graphiques d'analyse viendront dans une autre version.</li>
			...
			<li>Ajout d'une méthode dans config pour extraire le nom de l'utilisateur de l'idCAS.<br>
				Sinon, par défaut : récupération du nom de l'utilisateur avec les info CAS "cn" ou "displayName". Si
				aucun de fonctionne, affichage de 'Mme, M.'.
			</li>
			<li>Ajout du groupe étudiant sur le relevé.</li>
			<li>Ajout d'une option dans config.php pour personnaliser le message en cas de non diffusion des relevés par
				Scodoc.</li>
			...
			<li>Utilisation de l'année universitaire fourni par Scodoc pour la liste des semestres qu'un étudiant à
				suivi.</li>
			...
			<br>
			<b>Corrections</b>
			<li>Correction bug potentiel d'authentification si l'idCAS étudiant ou enseignant est une fraction l'un de
				l'autre.</li>
		...
		<ul>
			<li>Amélioration du script installOrUpdate.sh : ajout de fonctionnalités pour l'installation et lors de la
				mise à jour, conservation des favicon.ico et images/icons/* pour une adaptation locale de ces fichiers.
			</li>
			<li>Utilisation en partie de la nouvelle API Scodoc.</li>
			<li>Utilisation des données Scodoc pour choisir un relevé étudiants => plus besoin de renseigner le fichier
				liste_etu.txt pour avoir l'autocomplétion en mode enseignant.</li>
			<li>Amélioration des l'affichage des données de cette listes.</li>
			<li>Correctif mineur d'affichage : le semestre affiché par défaut aux étudiants était celui en cours, mais
				l'affichage au niveau des choix montrait le S1.</li>
			<li>Amélioration des diagnostics.</li>
			<li>Amélioration de l'affichage des semestres aux étudiants : changement de styles, ajout des années, ajout
				du "vrai" semestre, etc.</li>
			<li>Corrections de bugs introduits par la nouvelle liste étudiants : affichage des photos en mode enseignant
				et affichage des absences de la passerelle aux étudiants.</li>
...
chmod +x installOrUpdate.sh</pre></code>
		...
		<p><b>⚠️⚠️⚠️ ATTENTION : IL EST NECESSAIRE MODIFIER LE RÔLE et DE LIER LE RÔLE AUX PERMISSIONS DANS SCODOC (si
				ce n'est pas déjà fait) ⚠️⚠️⚠️</b></p>
		<p><i>==> Ajoutez le rôle LecteurAPI au compte qui se connecte à l'API (pour le moment il faut laisser le rôle
				Secr car la passerelle utilise en partie l'ancienne API)</i></p>
		<p>
			<i>
...
flask edit-role -a ScoView LecteurAPI</pre></code>
		...
		<ul>
			<li>Mise en place du numéro de version dans le fichier defaut_config pour l'avoir accessible dans l'ensemble
				du code.</li>
			<li>Identification de la passerelle auprès de Scodoc : ajout de l'entête HTTP referer avec l'URL du serveur
				et la version de la passerelle pour la communication avec Scodoc.</li>
		</ul>
		<h2>24/04/2022 - 4.7.13</h2>
		<ul>
			<li>Mise en place d'un système de désactivation du cache agressif : ajouter <code>?-no-sw</code> dans l'URL.
			</li>
			<li>Amélioration des diagnostics page 2 :
				<ul>
					<li>Mise en place d'une page debug du CAS : <code>/code_test/testCAS.php?-no-sw</code></li>
					<li>Amélioration des tests et conseils
					<li>
				</ul>
			</li>
		...
		<p>
			...
			Ce script est compatible Ubuntu et Debian, il permet lors d'une première installation d'installer tout le
			nécessaire sur le serveur, il reste alors à configurer les fichiers /config/*<br><br>

			Lorsque le serveur est déjà opérationnel, il permet de faire une mise à jour de /html, /includes et
			/lib.<br>
...
./installOrUpdate.sh</code></pre>
		...
		Le script accepte comme paramètre un chemin différent afin de permettre la mise à jour pour ceux qui ont
		configurer des Virtual Hosts.
		<code>./installOrUpdate.sh cheminVersLaPasserelle</code>
		</p>
		<h2>20/04/2022 - V4.7.12</h2>
		<ul>
			<li>Fichier config.php - Ligne 12 - $nom_IUT : possibilité de choisir le nom de l'IUT, si le fichier config
				n'est pas modifié, ce sera par défaut 'IUT'.</li>
		</ul>
		<h2>01/04/2022 - V4.7.11</h2>
		<ul>
			<li>Suppression des chmod qu'il restait dans le code : attention, /data/* doivent appartenir à www-data.
			</li>
		</ul>
		...
		<ul>
			<li>Changement de l'indication textuelle pour les justifications d'absences et ainsi coller au code du
				travail - avant, 48h après le retour pour justifier - maintenant, 48h après le début de l'absence pour
				justifier.</li>
		</ul>
		<h2>04/03/2022 - V4.7.7</h2>
		<div>!!! <b>Attention</b>, pour cette mise à jour, si vous utilisez le LDAP, il faut vérifier que les variables
			$LDAP_idCAS et $LDAP_autocompletion soient conformes à votre configuration.<br>Par défaut, leur valeur est à
			'mail'.</div>
		<ul>
			...
			<li>LDAP : possibilité de choisir un champ LDAP différent que l'idCAS pour l'autocomplétion - attention, ne
				pas modifier si $CAS_return_type != 'nip' - pour plus d'info, voir dans le fichier config.php</li>
		</ul>
		...
		<ul>
			...
			<li>Possibilité de modifier, à partir du fichier config, les photos renvoyées par l'API => function
				customPic()</li>
			<li>Possibilité de modifier, à partir du fichier config, les data générés par l'API avant l'envoie =>
				function customOutput()</li>
			<li>Ajout d'une zone "custom", remplie au choix de chaque IUT, depuis le fichier config => voir function
				customOutput().</li>
		</ul>
		...
		<ul>
			...
			<li>Correction bug : données vides dans le téléchargement des fichiers XLSX dans les listes d'étudiants.
			</li>
		...
		<ul>
			...
			<li>Liste des départements générés automatiquement depuis Scodoc (et plus dans le fichier config) en
				utilisant la nouvelle API.</li>
			<li>Lors du clic sur un étudiant dans le trombinoscope ou sur les listes : affichage de son relevé de notes.
			</li>
			<li>Modification du filtrage LDAP : possibilité de ne pas filtrer par UFR - contribution : Marc Leforestier
				(Bordeaux)</li>
		...
		<ul>
			...
			<li>Refonte complète du système côté serveur : passage du code en POO, réorganisation des fichiers, etc.
			</li>
			...
			<li>Lise en place d'un système de gestion des photos étudiants : l'étudiant gère sa propre photo. Les photos
				serveur pour les trombinoscopes et les absences.</li>
			<li>Mise en place d'un système pour un versionnage du fichier de configuration et d'une configuration par
				défaut.</li>
		</ul>

		<h2>04/01/2021 - V3.0.1</h2>
		<ul>
			<li class=txt-barre>Création d'un <a target=_blank
					href="https://notes.iutmulhouse.uha.fr/?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzZXNzaW9uIjoiQ29tcHRlX0RlbW8udGVzdEB1aGEuZnIiLCJzdGF0dXQiOiJldHVkaWFudCJ9.kHuiNx8X2mWUjv1LAHVOdcLGCu2yQS_i6fxqZZICuEA">compte
					démo</a></li>
			<li>Mise en ligne du code source sur <a target=_blank
					href="https://github.com/SebL68/Scodoc_Notes">GitHub</a></li>
		</ul>
		...
		<ul>
			...
			<li>Optimisation des performances : gzip requêtes, réduction du nombre de requêtes envoyées aux services
				(une à la place de trois) et du serveur à Scodoc.</li>
			...
			<li>Génération automatique des fichiers Excel pour les listes d'émargements, les groupes d'étudiants, le
				retour des notes, les données des étudiants.</li>
		...
		<p>
		<ul>
			<li>Amélioration de la détection d'erreurs (ajout du cas où le NIP est erroné et de la non autorisation de
				l'export des notes dans la configuration du semestre).</li>
		</ul>
		</p>
		...
		<p>
		<ul>
			<li>Correction d'un bug affichant un statut de réussite semi vide pour les étudiants en cours de cursus.
			</li>
