...
?>">
	...
	<main>
		<a href="avatar.php" aria-label="Changer la photo">
			<img alt="Photo de profil" class=studentPic
				src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" width=350 height=450>
		</a>
		<p>
		...
		<div class=etudiantHide>
			Vous êtes un personnel de l'IUT , <input required list=etudiants name=etudiant
				placeholder="Choisissez un étudiant" onchange="loadSemesters(this);this.blur()">
		...
		<div class="absences">
			...
			<p><i>
					Les causes de l’absence doivent être notifiées par écrit à l'aide d'un justificatif dans les 48
					heures à compter du début de l’absence au secrétariat du département. Voir règlement intérieur pour
		...
		<hr>
		<small>Ce site utilise deux cookies permettant l'authentification au service et une analyse statistique
			anonymisée des connexions ne nécessitant pas de consentement selon les règles du RGPD.</small><br>
		<small>Application réalisée par Sébastien Lehmann, enseignant MMI à l'IUT de Mulhouse - <a
				href="maj.php?-no-sw">version
				<?php echo $Config->passerelle_version; ?>
			</a> - <a href="https://github.com/SebL68/Scodoc_Notes">code source</a></small>
	</main>
	...
	<script>
		...
		function showReportCards(data, semestre) {
			if (data.relevé.publie == false) {
				document.querySelector(".releve").innerHTML = "<h2 style='background: var(--accent);'>" + data.relevé.message + "</h2>";
